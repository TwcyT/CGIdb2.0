...
from .models import SL_SV_Table, Gene, SL_SV_Table_detail, Complex, Pathway, Chemical, TCGA_info, gene_tcga_alter_samples,SL_Table_A,SL_Table_B,SL_Table_C,SL_Table_D,SL_Table_E,SL_Table_F,SL_Table_G,SL_Table_H,SL_Table_I,SL_Table_J,SL_Table_K,SL_Table_L,SL_Table_M,SL_Table_N,SL_Table_O,SL_Table_P,SL_Table_Q,SL_Table_R,SL_Table_S,SL_Table_T,SL_Table_U,SL_Table_V,SL_Table_W,SL_Table_X,SL_Table_Y,SL_Table_Z,SL_Table_other,SV_Table_A,SV_Table_B,SV_Table_C,SV_Table_D,SV_Table_E,SV_Table_F,SV_Table_G,SV_Table_H,SV_Table_I,SV_Table_J,SV_Table_K,SV_Table_L,SV_Table_M,SV_Table_N,SV_Table_O,SV_Table_P,SV_Table_Q,SV_Table_R,SV_Table_S,SV_Table_T,SV_Table_U,SV_Table_V,SV_Table_W,SV_Table_X,SV_Table_Y,SV_Table_Z,SV_Table_other
...
def search(request):
    SL_SV_pairs = SL_SV_Table_detail.objects.filter(Q(tissue='biliary_tract'))
    SL_SV_pairs_length = SL_SV_Table_detail.objects.filter(Q(tissue='biliary_tract')).count()
    for s in SL_SV_pairs:
        s.source = s.source.strip().split(";")
    return HttpResponse(loader.get_template('search.html').render({"start":1,"browse_pairs_search":SL_SV_pairs, "browse_length_search":SL_SV_pairs_length},request))
...
def browser_gene_SL_orgin(request, letter):
    template = loader.get_template("browse_gene_SL.html")
    SL_SV_pairs = Gene.objects.filter(gene_symbol__startswith=letter)[:10]
    SL_SV_pairs_length = Gene.objects.filter(gene_symbol__startswith=letter).count()
...
def browse_gene_SL(request,letter):
    SL_SV_pairs = SL_SV_Table.objects.filter(Q(gene1_symbol__startswith=letter) & Q(type='SL'))
    SL_SV_pairs_length = SL_SV_Table.objects.filter(Q(gene1_symbol__startswith=letter) & Q(type='SL')).count()
    for s in SL_SV_pairs:
        s.source = s.source.strip().split(";")
    return HttpResponse(loader.get_template("browse_gene_SL.html").render({"start":1,"browse_pairs":SL_SV_pairs, "browse_length":SL_SV_pairs_length}, request))

def browse_gene_SV(request,letter):
    SL_SV_pairs = SL_SV_Table.objects.filter(Q(gene1_symbol__startswith=letter) & Q(type='SV'))
...
def browse_tissue(request,tissue):
    ...
    for s in SL_SV_pairs:
        s.source = s.source.strip().split(";")
    return HttpResponse(loader.get_template("browse_tissue_orgin.html").render({"start":1,"browse_pairs":SL_SV_pairs, "browse_length":SL_SV_pairs_length}, request))
